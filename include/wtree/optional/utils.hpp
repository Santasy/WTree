...
class WTreeValidationUtils {
  public:
            ...
            for(field_type i = 0; i < node->size() - 1; ++i) {
                if(!(node->key(i) < node->key(i + 1))) {
                    if constexpr(std::is_arithmetic_v<key_type>) {
                        printf(
                            "[VERIFY ERROR :: Order] Not in order: %lu %lu\n",
    ...
    /**
     * @brief Validates the iterator path (as validate_iterator_path) and
     * additionally checks the mapped value at each node slot.
     * @param it        The iterator whose path is validated.
     * @param validator A callable (key, value) -> bool returning true when the
            ...
            for(field_type i = 0; i < node->size(); ++i) {
                ...
                if(!validator(entry.first, entry.second)) {
                    ...
                    printf("On node %lu/%lu of path, index %lu\n",
                           (ulong)i_node + 1, (ulong)it.ascendants.size(),
                           (ulong)i);
                    return false;
                }
            }
        ...
        for(field_type i = 0; i < node->size() - 1; ++i) {
            ...
            if(!(node->key(i) < w->key(0)) ||
               !(w->key(w->size() - 1) < node->key(i + 1))) {
                if constexpr(std::is_arithmetic_v<key_type>) {
                    printf(
                        "[VERIFY ERROR :: Not bounded] Descendant bounds are "
                        "not correct:\n"
                        "\t(u=%lu, w=%lu) , (w=%lu, u=%lu)\n",
                        (ulong)node->key(i), (ulong)w->key(0),
