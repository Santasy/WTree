...
template <typename WTreeType> class WTreePrinter {
  ...
  private:
    ...
    std::string format_all_keys(const node_type *node, int indent) const {
        ...
        for(int i = 0; i < node->size(); ++i) {
            ...
            if(options.max_values_per_line > 0 &&
               (i + 1) % options.max_values_per_line == 0 &&
               i + 1 < node->size()) {
                oss << "\n" << std::string(indent + options.indent_size, ' ');
            }
        }
    ...
    void print_children_improved(const node_type *node,
...
class PrintableContainer : public ContainerType {
  ...
  public:
    ...
    void set_print_options(const PrintOptions &opts) {
        m_printer.options = opts;
    }
    ...
    void print_compact() const {
        m_printer.print_compact(this->tree()->root());
    }
