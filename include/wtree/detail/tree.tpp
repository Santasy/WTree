...
WTree<Params>::WTree(const key_compare &comp, const allocator_type &alloc)
    ...
    if(real_bytes > kTargetNodeBytes) {
        if(kInitialCapacity == 3) {
            std::printf(
                "\t[WARNING] Nodes are %lu bytes instead of %u bytes to "
                "have 3 values.\n",
...
WTree<Params>::emplace_unique_key_args(const key_type &key,
    ...
    if(m_locator.internal_locate_any(key, it) == static_cast<int>(kExactMatch))
        return std::make_pair(it, false);

    // R2: Node not full — emplace directly.
    if(it.node->size() < kTargetK)
        return std::make_pair(m_manager.internal_emplace_at_with_growth(
    ...
    if(it.node->is_leaf()) {
        ...
        if constexpr(params_type::use_slide && params_type::use_split) {
            correct_attempt =
                m_manager.attempt_emplace_with_balanced_slide_and_split(
                    it, std::forward<Args>(args)...);
        } else if constexpr(params_type::use_slide) {
            correct_attempt = m_manager.attempt_emplace_with_balanced_slide(
                it, std::forward<Args>(args)...);
        } else if constexpr(params_type::use_split) {
            correct_attempt = m_manager.attempt_emplace_with_balanced_split(
                it, std::forward<Args>(args)...);
        }
...
WTree<Params>::emplace_hint_unique_key_args(const_iterator hint,
    ...
    // which preserves std semantics (hint affects only complexity).
...
Iterator WTree<P>::erase(Iterator &iter) {
    ...
    if(iter.node->is_internal()) {
        field_type child_lower_bound =
            iter.node->first_left_descendant(iter.index);
        ...
        if(child_lower_bound > 0) {
            ...
            if(child_node.node->is_internal()) {
                ...
                node_type *ancestor = child_node.has_ascendant()
                                          ? child_node.ascendant()
                                          : iter.node;
            ...
            if(child_node.node->size() == 0) {
                m_manager.internal_destroy_child_unchecked(
                    iter.node, child_lower_bound - 1);
            }
