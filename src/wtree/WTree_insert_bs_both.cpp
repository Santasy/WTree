...
template <typename T> bool WTree<T>::_insertFromRoot(T key) {
  ...
  if (root->_fields.n < node_type::c_target_k) {
    if (root->_fields.n > 0) {
      ...
      T *anchor = root->_fields.values;
      typename node_type::field_type len = root->_fields.n, half;
      while (len > 1) {
        half = len >> 1;
        anchor += (anchor[half - 1] < key) * half;
        len -= half;
      }
  ...
  run_internal:
    ...
    // R4: SplitSide
    if (trySplit(u, ui, p, pi, key) == true)
      return true;
