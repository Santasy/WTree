  ...
  protected:
    ...
    BaseGenerator &operator=(const BaseGenerator &other) {
        if(this != &other) {
            ...
            std::copy(other.visited, other.visited + get_bitvector_size(),
                      visited);
        }
        return *this;
    }
    ...
    void base_init() {
        ...
        visited = new ulong[get_bitvector_size()](); // The parenthesis sets
                                                     // values to 0.
    }
...
class GaussianGenerator : public BaseGenerator<IntType> {
  ...
  private:
    static IntType calculate_max_value(IntType mean, IntType std) {
        ...
        // Check for overflow: if mean > type_max - 5*std, then mean + 5*std
        // would overflow
    ...
    static IntType calculate_min_value(IntType mean, IntType std) {
        ...
        if constexpr(std::is_unsigned_v<IntType>) {
            ...
            // We need to be careful with the arithmetic to avoid overflow in
            // the check itself If mean - type_min < 5*std, then mean - 5*std
            // would underflow
  ...
  public:
    explicit GaussianGenerator(size_t seed = 0, IntType mean = 0,
                               IntType std = 1)
